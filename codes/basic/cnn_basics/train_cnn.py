...
def compute_accuracy(v_xs,v_ys):
    global prediction
    pred=sess.run(prediction,feed_dict={inputX:v_xs,keep_prob:1.0})
    correct_prediction = tf.equal(tf.argmax(pred, 1), tf.argmax(v_ys, 1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    result = sess.run(accuracy, feed_dict={inputX: v_xs, inputy: v_ys, keep_prob: 1})
    return result
...
with tf.variable_scope("cnn"):
    ...
    prediction,variables=cnn(inputX,keep_prob)
...
for i in range(10000):
    ...
    l,_=sess.run([loss,optimizer],feed_dict={inputX:train_x,inputy:train_y,keep_prob:0.8})
...
save_path = saver.save(sess, "my_net/cnn_net.ckpt")
